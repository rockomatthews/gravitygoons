...
if (stage === "mainnet") {
  if (process.env.ALLOW_MAINNET_MATCH_ESCROW_DEPLOY !== "true") throw new Error("Set ALLOW_MAINNET_MATCH_ESCROW_DEPLOY=true only for the reviewed deployment checkpoint");
...
const artifact = JSON.parse(fs.readFileSync(path.join(root, "artifacts", "GoonMatchEscrow.json"), "utf8"));
const escrow = await new ContractFactory(artifact.abi, artifact.bytecode, deployer).deploy(
  safe, usdc, collection, settlementSigner, safe,
);
...
if (verified.owner !== safe || verified.feeRecipient !== safe || verified.usdc !== usdc || verified.collection !== collection || verified.settlementSigner !== settlementSigner) throw new Error("Escrow constructor verification failed");
if (!verified.paused || verified.houseFeeBps !== 0) throw new Error("Escrow must deploy paused with a zero-percent fee");

const recordPath = path.join(repositoryRoot, "reports", `base-${stage}-match-escrow-deployment.json`);
if (fs.existsSync(recordPath) && process.env.ALLOW_DEPLOYMENT_RECORD_OVERWRITE !== "true") throw new Error(`Refusing to overwrite ${recordPath}`);
const record = {
  schema: "gravity-goons-match-escrow-deployment-v1",
...
console.log(`GoonMatchEscrow: ${address}`);
console.log(`Deployment record: ${recordPath}`);
console.log("Escrow is PAUSED. No USDC can be funded until the Safe explicitly calls unpause().");
