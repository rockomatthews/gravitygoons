...
export function ArenaMatchView({ matchId }: { matchId: string }) {
  ...
  return <>
    ...
    <section className="broadcast-panels">
      ...
      <article><span>PLAYER STAKES + SPECTATOR MARKET</span><h2>{detail.wager.enabled ? detail.wager.state : "LOCKED"}</h2><p>{detail.wager.notice}</p><dl><div><dt>PLAYER USDC</dt><dd>{detail.wager.stakeMinor ? `$${detail.wager.stakeMinor / 1_000_000}` : "LOCKED"}</dd></div><div><dt>PINK SLIP</dt><dd>LOCKED</dd></div><div><dt>HOUSE FEE</dt><dd>{(detail.wager.houseFeeBps / 100).toFixed(2)}%</dd></div><div><dt>SPECTATORS</dt><dd>{market?.collateral ?? "PLAY"}</dd></div></dl><p className="pink-slip-broadcast-lock"><b>PINK SLIP — WINNER TAKES BOTH GOONS</b><span>Separate NFT escrow, legal clearance, and independent audit required.</span></p>{market && <div className="partner-market-preview"><b>{market.provider} · {market.mode.toUpperCase()}</b>{market.outcomes.map((outcome) => <span key={outcome.athleteName}>{outcome.athleteName} <strong>{Math.round(outcome.price * 100)}%</strong></span>)}<small>{market.notice}</small>{market.tradeUrl && market.tradingEnabled && <a href={market.tradeUrl} target="_blank" rel="noreferrer">OPEN APPROVED PARTNER →</a>}</div>}</article>
