...
export function ArenaMatchView({ matchId }: { matchId: string }) {
  ...
  return <>
    ...
    <section className="broadcast-panels">
      ...
      <article><span>PLAYER STAKES + SPECTATOR MARKET</span><h2>{detail.wager.requested ? detail.wager.state.replaceAll("_", " ").toUpperCase() : "NO WAGER"}</h2><p>{detail.wager.notice}</p><dl><div><dt>PLAYER USDC</dt><dd>{detail.wager.stakeMinor ? `$${detail.wager.stakeMinor / 1_000_000} EACH` : "NONE"}</dd></div><div><dt>PINK SLIP</dt><dd>LOCKED</dd></div><div><dt>HOUSE FEE</dt><dd>{(detail.wager.houseFeeBps / 100).toFixed(2)}%</dd></div><div><dt>SPECTATORS</dt><dd>{market?.collateral ?? "PLAY"}</dd></div></dl>{detail.wager.requested && detail.wager.enabled && detail.wager.terms && [detail.wager.terms.playerA.toLowerCase(), detail.wager.terms.playerB.toLowerCase()].includes(account?.toLowerCase() ?? "") && detail.wager.state !== "locked" && detail.wager.state !== "result_proposed" && detail.wager.state !== "settled" && <button onClick={fundWager} disabled={funding}>{funding ? "CHECK WALLET…" : "LOCK MY USDC STAKE"}</button>}{detail.wager.requested && ["created", "partially_funded", "refund_pending"].includes(detail.wager.state) && detail.wager.fundingDeadline && clock > Date.parse(detail.wager.fundingDeadline) && <button onClick={() => escrowAction("refundExpired")} disabled={funding}>REFUND EXPIRED FUNDING</button>}{detail.wager.state === "result_proposed" && detail.wager.chain && clock <= detail.wager.chain.disputeDeadline * 1000 && <button onClick={() => escrowAction("dispute")} disabled={funding}>DISPUTE PROPOSED RESULT</button>}{detail.wager.state === "result_proposed" && detail.wager.chain && clock > detail.wager.chain.disputeDeadline * 1000 && <button onClick={() => escrowAction("finalize")} disabled={funding}>FINALIZE PAYOUT</button>}<p className="pink-slip-broadcast-lock"><b>PINK SLIP — WINNER TAKES BOTH GOONS</b><span>Separate NFT escrow; not active.</span></p>{market && <div className="partner-market-preview"><b>{market.provider} · {market.mode.toUpperCase()}</b>{market.outcomes.map((outcome) => <span key={outcome.athleteName}>{outcome.athleteName} <strong>{Math.round(outcome.price * 100)}%</strong></span>)}<small>{market.notice}</small>{market.tradeUrl && market.tradingEnabled && <a href={market.tradeUrl} target="_blank" rel="noreferrer">OPEN APPROVED PARTNER →</a>}</div>}</article>
