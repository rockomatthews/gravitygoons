...
export function RankedMatch({ matchId }: { matchId: string }) {
  ...
  return <section className="ranked-match">
    ...
    <div className="ranked-strategy" aria-label="Turn strategy">
      <div><span>CALL MODE</span><div className="ranked-mode-buttons"><button className={callMode === "standard" ? "active" : ""} disabled={!isLive || !match.viewer_is_setter || submitting || turnExpired} onClick={() => setCallMode("standard")}>STANDARD</button><button className={callMode === "send" ? "active" : ""} disabled={!isLive || !match.viewer_is_setter || submitting || turnExpired || setterGrit <= 0} onClick={() => setCallMode("send")}>SEND IT · 1 GRIT</button></div><small>SEND IT: setter −10% · automatic responder −15%</small></div>
