...
export async function POST(request: Request) {
  try {
    const body = await request.json() as { tokenId?: number; discipline?: Discipline };
    ...
    return NextResponse.json(await startTrickLine({ wallet, tokenId: body.tokenId, discipline: body.discipline }), { status: 201 });
  } catch (error) {
