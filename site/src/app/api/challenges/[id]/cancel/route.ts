...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    return NextResponse.json(await transitionChallenge(wallet, id, "cancel", body.issuedAt, body.signature));
  } catch (error) {
    const message = error instanceof Error ? error.message : "Unable to cancel challenge.";
